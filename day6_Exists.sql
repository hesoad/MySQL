...
select musteri_isim,urun_isim
from nisan_satislar
where not exists (select urun_isim from mayis_satislar where mayis_satislar.urun_isim=nisan_satislar.urun_isim);


/*----------------------------------------------------------------
SORU  :mayis ayinda satilip nisan ayinda satilmayan urun id  ve
satin alan kisiyi listeleyen bir QUERY yaziniz.
----------------------------------------------------------------*/
select musteri_isim, urun_id
from mayis_satislar
where not exists (select urun_id from nisan_satislar where mayis_satislar.urun_isim=nisan_satislar.urun_isim);
...
-- ismi null olan kayitlari isimsiz olarak gunceleyin
...
from insanciklar
order by ssn desc;
-- Soru 5: Insanciklar tablosundaki tum kayitlari Soyisimler ters sirali,-- isimler Natural sirali olarak listeleyin.
select *
from insanciklar
...
-- Soru 8: Insanciklar tablosundaki tum kayitlari SSN numarasina gore siralayin.
select * 
from insanciklar
order by ssn;
